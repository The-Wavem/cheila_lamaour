...
import { buildTimelineFromRange, filterTimestampedRecords, resolveFilterRange } from './analyticsDateRange';
...
export const getHomeAccessStats = async (options = {}) => {
    try {
        const q = query(collection(db, PAGE_ACCESS_COLLECTION), orderBy('createdAt', 'desc'));
        ...
        const totalGeneralAccesses = homeAccesses.filter((item) => item.accessType === 'general').length;
